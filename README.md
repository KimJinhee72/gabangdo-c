...
카카오 지도 버셀에 나오려면
1.kakao(api)개발자로그인 후 일반에 앱키 중 스크립트용을 로컬에 .env 만들어 VITE_KAKAO_MAP_KEY=JavaScript 키 넣고
...
5. 지도가 혹시 안보이면 카카오 버전 문제로 그럴수 있으니 <client-only><파일명/></client-only> client-only 안에 <파일명/> 넣으면 버셀에서도 지도가 보임
