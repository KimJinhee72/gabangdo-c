  ...
  <div class="w-[390px] bg-gray-900 min-h-screen flex justify-center py-3 m-auto">
    <div class="w-[350px] mx-auto">
      ...
      <div class="flex justify-between items-center mb-4 mt-2">
        <div class="logo">
          <router-link to="/worker/ddashboard">
            <img src="/images/yr/delivery/logoL.png" alt="로고" class="w-15 h-5" />
          </router-link>
        </div>
        <div class="flex gap-2">
          <router-link to="/worker/assigned-jobs">
            <img src="/images/yr/delivery/check_icon.png" alt="체크" class="w-5 h-5" />
          </router-link>
      ...
      <div class="bg-white rounded-lg shadow space-y-3 mb-2">
        <div class="flex justify-between items-center pl-4 pr-4 pt-1.5 pb-1.5 bg-gray-200 rounded-t-2xl rounded-b-none">
          <div class="flex items-center space-x-3">
            <img src="/images/yr/delivery/profile.png" alt="" />
            <div>
              <p class="font-semibold">이준호 기사님</p>
            </div>
            <div class="w-8 h-8 rounded-full flex items-center justify-center text-white text-xs mt-[1px]">
      ...
      <div class="bg-white rounded-lg shadow">
        ...
        <div class="flex flex-wrap gap-2 pl-4 pr-4 pb-4 pt-2 mb-2">
          <button @click="toggleLuggage('소형')"
            ...
            <img :src="isActive('소형') ? '/images/yr/delivery/check_white.png' : '/images/yr/delivery/cancel_white.png'"
              :class="['icon w-4', isActive('소형') ? 'h-5 mr-1' : 'h-4 mr-1 mt-[2.2px]']" alt="아이콘" />
            <span>소형</span>
          </button>

          <button @click="toggleLuggage('중형')"
            ...
            <img :src="isActive('중형') ? '/images/yr/delivery/check_white.png' : '/images/yr/delivery/cancel_white.png'"
              :class="['icon w-4', isActive('중형') ? 'h-5 mr-1' : 'h-4 mr-1 mt-[2.2px]']" alt="아이콘" />
            <span>중형</span>
          </button>

          <button @click="toggleLuggage('대형')"
            ...
            <img :src="isActive('대형') ? '/images/yr/delivery/check_white.png' : '/images/yr/delivery/cancel_white.png'"
              :class="['icon w-4', isActive('대형') ? 'h-5 mr-1' : 'h-4 mr-1 mt-[2.2px]']" alt="아이콘" />
            <span>대형</span>
          </button>

          <button @click="toggleLuggage('특대형')"
            ...
            <img :src="isActive('특대형') ? '/images/yr/delivery/check_white.png' : '/images/yr/delivery/cancel_white.png'"
              :class="['icon w-4', isActive('특대형') ? 'h-5 mr-1' : 'h-4 mr-1 mt-[2.2px]']" alt="아이콘" />
            <span>특대형</span>
          </button>
        </div>
      </div>
      ...
      <div class="bg-white rounded-lg shadow px-4 py-3 mb-2">
        ...
        <div class="flex gap-2 items-end justify-center">
          <button @click="togglePreference('공항')" :class="[
          ...
          <button @click="togglePreference('기차역')" :class="[
          ...
          <button @click="togglePreference('숙소')" :class="[
      ...
      <div class="bg-white rounded-lg shadow space-y-2 mb-2">
        <div class="flex justify-between pb-2 px-4 py-3">
          ...
          <button>
            <img src="/images/yr/delivery/arrowL.png" class="w-4 h-4" alt="" />
          </button>
        </div>
        <div class="flex gap-2 px-4 py-1">
          <div class="w-full h-3 bg-gray-200 rounded-full relative">
            <div class="absolute top-0 left-0 h-3 bg-orange-500 rounded-full w-full" />
          </div>
        ...
        <div class="flex items-center justify-center gap-1 text-sm text-black bg-gray-200 rounded-b-lg py-1">
          <span>다음 교육 이수:</span>
      ...
      <!-- 하단 버튼 -->
      <div class="flex justify-between">
        <button class="flex items-center justify-center gap-1 bg-[#868686] w-[170px] h-[40px] rounded-lg text-sm">
          <img class="w-4 h-4" src="/images/yr/delivery/setting.png" alt="" />
          <p class="text-white">설정</p>
        </button>
        <button class="w-[170px] h-[40px] bg-[#279BF3] text-white px-6 py-2 rounded-lg text-sm font-medium">
          고객센터
        </button>
      </div>
    </div>
