...
const handleLogin = () => {
  console.log('로그인 시도:', {
    ...
    ipSecurity: loginForm.value.ipSecurity
  })

  // 실제 로그인 로직 구현
  alert(`${tabs.find(tab => tab.id === activeTab.value)?.label} 로그인 시도`)
}
</script>
      ...
      <div class="content">
        <div class="login_wrap">
          ...
          <ul class="menu_wrap" role="tablist">
            <li v-for="tab in tabs" :key="tab.value" class="menu_item" role="presentation">
              <a href="#" :id="tab.id" class="menu_id" :class="[tab.class, { on: activeTab === tab.value }]" role="tab"
                :aria-selected="activeTab === tab.value ? 'true' : 'false'" @click.prevent="activeTab = tab.value">
                <span class="menu_text">
                  <img v-if="tab.icon" :src="tab.icon" alt="" class="menu_icon" />
                  <span class="text">{{ tab.label }}</span></span>
              </a>
            </li>
          </ul>
          ...
          <form id="frmNIDLogin" name="frmNIDLogin" target="_top" autocomplete="off"
            ...
            <input type="hidden" name="dynamicKey" id="dynamicKey"
              value="fzc45_PhgW-s-D3ABNXqu3aiGCP_Ro7k_Df3rdpPSq7PAwOMZkJgs_KKLmRbGpB4bT_I4NttGC_32qJK3D0kb4mZRu9D5298NdWmkfA0Nb8">
                  ...
                  <div class="login_form">
                    <div class="login_box">
                      <div class="input_item id" id="input_item_id">
                        <input type="text" id="id" name="id" accesskey="L" maxlength="41" autocapitalize="none" value=""
                      ...
                      <div class="input_item pw" id="input_item_pw">
                        <input type="password" id="pw" name="pw" title="비밀번호" class="input_pw" maxlength="16"
                          aria-label="비밀번호">
                        <label for="pw" class="text_label" id="pw_label" aria-hidden="true">비밀번호</label>
                        <button type="button" class="btn_view hide" id="pw_hide" aria-pressed="false"
                          style="display: none;">
    ...
    <div class="footer">
      <div class="footer_inner">
        ...
        <ul class="footer_link" id="footer_link">
          ...
          <li><a target="_blank" class="footer_item nlog-click" href="http://www.naver.com/rules/privacy.html"
              id="fot.privacy"><span class="text"><strong>개인정보처리방침</strong></span></a></li>
          <li><a target="_blank" class="footer_item nlog-click" href="http://www.naver.com/rules/disclaimer.html"
              id="fot.disclaimer"><span class="text">책임의 한계와 법적고지</span></a></li>
          <li><a target="_blank" class="footer_item nlog-click"
...
.header_blank.svg .header_title .link::before {
  ...
  background-image: linear-gradient(transparent, transparent), url(https://ssl.pstatic.net/static/nid/login/sprite/sp_00_common_e9c7fba7.svg);
...
.popup .popup_content.type_loading {
  padding: 35px 30px
}
...
.popup .btn_popup.cancel {
  background: #8990a0
}

.popup .btn_popup.confirm {
  background: #09aa5c
}
...
.menu_ones.on .menu_text,
.menu_qr.on .menu_text {
  color: #333;
  font-weight: bold;
}
...
.menu_ones .menu_text::before {
  background-position: -316px -294px;
  background-repeat: no-repeat;
  width: 16px;
  height: 16px
}
...
.menu_ones.on .menu_text::before {
  background-position: -316px -276px;
  background-repeat: no-repeat;
  width: 16px;
  height: 16px
}
...
.input_item input:autofill+label {
  left: 7px;
  bottom: 30px;
  font-size: 12px;
  padding: 0 8px;
...
.input_item input:-webkit-autofill+label {
  left: 7px;
  bottom: 30px;
  font-size: 12px;
  padding: 0 8px;
...
.input_item input:-webkit-autofill:focus,
.input_item input:-webkit-autofill:hover {
  font-size: 16px !important
}

.input_item input::-ms-reveal {
  display: none
}
...
.input_item.pw.focus::after {
  border-radius: 0 0 8px 8px
}

.input_item.focus::after {
  content: '';
  position: absolute;
  top: -1px;
  left: -1px;
...
.input_item.country.id {
  padding-left: 94px
}

.input_item.country .text_label {
  bottom: 30px;
  font-size: 12px;
  left: 7px;
  padding: 0 8px;
...
.input_item.country .country_code {
  position: absolute;
  bottom: 9px;
  left: 15px;
  min-width: 73px;
...
.input_item.country .country_select {
  position: relative;
  padding-right: 20px;
  font-size: 16px;
  font-weight: 400;
...
.input_item.country .input_id,
.input_item.country .input_pw {
  display: inline-block;
  box-sizing: border-box
}
...
.input_item.passkey.focus::after {
  border-radius: 8px
}
...
.country_list_area::-webkit-scrollbar {
  display: none
}
...
.country_list .country_item:hover {
  background-color: #e1e3e5
}

.country_list .country_item.check_bg {
  background-color: #f5f6f7
}

.country_list .country_item.check_bg:hover {
  background-color: #e1e3e5
}

.country_list .country_item .text {
  font-size: 15px;
  font-weight: 400;
  line-height: 21px;
  letter-spacing: -.4px;
...
.country_list .country_name .text {
  word-break: keep-all
}

.country_list .country_number {
  flex: none;
  padding-left: 6px
}
...
.captcha_wrap .captcha_img::after {
  content: '';
  position: absolute;
  top: 0;
  left: 0;
...
.captcha_wrap .audio_message .accent {
  color: #09aa5c
}

.captcha_wrap .captcha_audio {
  width: 100%;
  padding: 43px 0 42px;
  border-radius: 6px;
...
.captcha_wrap .captcha_audio .audio_text {
  padding-top: 14px;
  font-size: 16px;
  font-weight: 500;
  line-height: 23px;
...
.captcha_row .captcha_input {
  padding: 10px 15px
}

.captcha_row .btn_captcha {
  position: absolute;
  top: 0;
  right: 0
}

.captcha_row .btn_captcha::after {
  content: " ";
  display: block;
  clear: both
}

.captcha_row .btn_picture,
.captcha_row .btn_refresh,
.captcha_row .btn_sound {
  overflow: hidden;
  float: left;
  width: 44px;
  height: 44px;
...
.captcha_row .btn_picture+.btn_picture,
.captcha_row .btn_picture+.btn_sound,
.captcha_row .btn_refresh+.btn_picture,
.captcha_row .btn_refresh+.btn_sound,
.captcha_row .btn_sound+.btn_picture,
.captcha_row .btn_sound+.btn_sound {
  margin-left: 6px
}
...
.login_error_wrap.addchatbot {
  display: block;
  padding-right: 40px
}

.login_error_wrap.addchatbot .error_message {
  display: block
}

.login_error_wrap.addchatbot .chatbot {
  position: absolute;
  top: 50%;
  right: 0;
  margin-top: -15px
...
.step_wrap .step_top li:nth-child(3) .step_desc {
  margin: 15px 0 0;
  filter: drop-shadow(0 17px 14px rgba(36, 62, 98, .7))
}
...
.reconfirm_sub .captcha_wrap .captcha_pic {
  max-height: 194px
}
...
.reconfirm_sub .captcha_row {
  margin-top: 11px
}

.reconfirm_bottom .footer_copy .corp,
...
.reconfirm_bottom .footer_copy .corp+.footer_help,
.reconfirm_bottom .footer_copy .text+.footer_help {
  position: relative;
  padding-left: 13px
}

.reconfirm_bottom .footer_copy .corp+.footer_help::before,
.reconfirm_bottom .footer_copy .text+.footer_help::before {
  content: '';
  position: absolute;
  top: 50%;
  left: 5px;
...
#frmNIDLogin:has(#pw:-webkit-autofill) #log\.login {
  background-color: #09aa5c
}
...
body.oauth_mobile .container,
body.oauth_mobile .content,
body.oauth_mobile .oauth_wrap {
  height: 100%
}

body.oauth_mobile .header_oauth {
  box-sizing: border-box;
  max-width: 640px;
  margin: 0 auto;
  padding: 13px 15px
...
body.oauth_mobile .header_oauth .title_wrap {
  margin-left: -7px;
  padding: 10px 0 7px 38px
}

body.oauth_mobile .header_oauth .logo .link {
  padding: 7px
}

body.oauth_mobile .header_oauth .logo .link::before {
  background-position: -78px -266px;
  background-repeat: no-repeat;
  width: 24px;
  height: 24px
}

body.oauth_mobile .header_oauth .main_title {
  font-size: 18px;
  font-weight: 500;
  line-height: 21px;
  letter-spacing: -.5px;
...
body.oauth_mobile .header_oauth .profile_area {
  padding-top: 10px;
  padding-right: 4px
}

body.oauth_mobile .header_oauth .image_wrap {
  width: 20px;
  height: 20px
}

body.oauth_mobile .header_oauth .btn_profile {
  height: 20px;
...
body.oauth_mobile .oauth_wrap.full .top_area {
  padding-top: 15px;
  padding-bottom: 11px
}

body.oauth_mobile .oauth_wrap.full .scroll_area {
  padding-top: 0;
  font-size: 0;
  text-align: center
}

body.oauth_mobile .oauth_wrap .popup_area {
  display: flex;
...
body.oauth_mobile .oauth_wrap .popup {
  max-height: 460px;
  border-radius: 12px
}

body.oauth_mobile .oauth_wrap .image {
  width: 30px;
  height: 30px;
  border-radius: 10px
}

body.oauth_mobile .oauth_wrap .image::after {
  border-radius: 10px;
  border-width: .5px
}

body.oauth_mobile .oauth_wrap .name {
  padding-top: 3px;
  font-size: 18px;
  line-height: 22px
}

body.oauth_mobile .oauth_wrap .scroll_area {
  padding: 14px 20px 0
}

body.oauth_mobile .oauth_wrap .age_area {
  padding: 30px 0
}
...
body.oauth_full .header_oauth {
  width: 540px;
  margin: 0 auto;
  padding-top: 46px
}

body.oauth_full .header_oauth .header_inner {
  width: 500px;
  margin: 0 auto
}

body.oauth_full .oauth_wrap {
  width: 540px;
  margin: 58px auto 0;
  padding-bottom: 20px
}

body.oauth_full .oauth_wrap.full .popup {
  height: 625px
}

body.oauth_full .oauth_wrap.full .top_area {
  padding-top: 20px;
  padding-bottom: 13px
}

body.oauth_full .oauth_wrap.full .scroll_area {
  padding-top: 0
}

body.oauth_full .oauth_wrap .popup_area {
  width: 500px;
  margin: 0 auto
}

body.oauth_full .oauth_wrap .popup {
  max-height: 625px
}
...
body.oauth_pc_popup .container,
body.oauth_pc_popup .content,
body.oauth_pc_popup .oauth_wrap {
  height: 100%
}

body.oauth_pc_popup .header_oauth {
  box-sizing: border-box;
  padding: 9px 20px
}
...
body.oauth_pc_popup .oauth_wrap.full .top_area {
  padding: 20px 20px 13px
}

body.oauth_pc_popup .oauth_wrap.full .scroll_area {
  padding-top: 0
}

body.oauth_pc_popup .popup_area {
  height: 100%
}

body.oauth_pc_popup .scroll_area {
  flex: 1 1 auto;
  padding: 15px 20px 0
}

body.oauth_pc_popup .btn_area {
  border-radius: 0
}
...
.header_oauth .header_inner {
  display: flex;
  position: relative
}

.header_oauth .title_wrap {
  flex: none;
  display: inline-block;
  position: relative;
  margin-left: -8px;
...
.header_oauth .tooltip {
  ...
  right: 0;
  z-index: 100;
  box-shadow: 2px 2px 8px 0 rgba(0, 0, 0, .06);
...
.header_oauth [role=checkbox]:focus-visible,
.header_oauth a:focus-visible,
.header_oauth button:focus-visible,
.oauth_bridge [role=checkbox]:focus-visible,
.oauth_bridge a:focus-visible,
.oauth_bridge button:focus-visible,
.oauth_wrap [role=checkbox]:focus-visible,
.oauth_wrap a:focus-visible,
.oauth_wrap button:focus-visible,
.terms_detail [role=checkbox]:focus-visible,
.terms_detail a:focus-visible,
.terms_detail button:focus-visible {
...
.oauth_wrap.full .popup {
  width: 100%
}

.oauth_wrap.full .scroll_area {
  flex: 1 1 auto
}
...
.oauth_wrap .title_area {
  position: relative;
  padding: 15px 0
}

.oauth_wrap .title_area::after {
  display: block;
  clear: both;
  content: ''
}
...
.oauth_wrap .scroll_area strong {
  font-weight: 600
}

.oauth_wrap .section+.section {
  margin-top: 14px;
  padding-top: 14px;
  border-top: 1px solid #e3e3e3
}

.oauth_wrap .section_title {
  display: block;
  font-size: 16px;
  line-height: 19px;
  letter-spacing: -.6px;
...
.oauth_wrap .section_title+.check_list {
  margin-top: 8px
}
...
.oauth_wrap .desc+.check_list {
  margin-top: 14px
}

.oauth_wrap .check_list {
  font-size: 0
}

.oauth_wrap .check_list .block {
  display: inline-block;
  vertical-align: top
}

.oauth_wrap .check_item {
  position: relative
}

.oauth_wrap .check_item.has_link .custom_checkbox {
  margin-right: 16px
}

.oauth_wrap .check_item+.check_item {
  margin-top: 8px
}

.oauth_wrap .check_item .title {
  margin-right: 2px;
  font-weight: 500;
  font-size: 14px;
  line-height: 20px;
...
.oauth_wrap .sub_list .sub_item input:checked+label .desc {
  color: #222
}

.oauth_wrap .sub_list .sub_item label {
  padding-left: 22px
}

.oauth_wrap .sub_list .sub_item .desc {
  color: #767678
}
...
.oauth_wrap .btn_area::after {
  display: block;
  clear: both;
  content: ''
}

.oauth_wrap .btn_area .cell:first-of-type {
  float: left;
  width: 50%;
  border-bottom-left-radius: 12px
}

.oauth_wrap .btn_area .cell:last-of-type {
  overflow: hidden;
...
.oauth_wrap .btn_area .btn:focus-visible {
  outline: 0 !important;
  border-color: #000
}

.oauth_wrap .btn_area .btn:disabled {
  background: #c9d1dc !important;
  cursor: default
}

.oauth_wrap .btn_area .btn.cancel {
  background: #a5adb8
}

.oauth_wrap .btn_area .btn.agree {
  background: #09aa5c
}
...
.oauth_wrap .age_area .break_landscape {
  display: none
}
...
.oauth_bridge .button.gray {
  background: #a5adb8
}

.oauth_bridge .button.green {
  background: #09aa5c
}

.oauth_bridge .button:first-of-type {
  border-radius: 0 0 0 16px
}

.oauth_bridge .button:last-of-type {
...
.terms_detail .bottom_area {
  padding: 20px 0
}

.terms_detail .btn_confirm {
  width: 100%;
  padding: 16px 0;
  border-radius: 8px;
...
@media (orientation: landscape) {
  ...
  body.oauth_mobile .header_oauth .logo {
    top: 2px
  }
  ...
  body.oauth_mobile .oauth_wrap .scroll_area.align_center::after {
    display: inline-block;
    width: 0;
    height: 100%;
    vertical-align: middle;
  ...
  body.oauth_mobile .oauth_wrap .age_area .title {
    margin-top: 0
  }

  body.oauth_mobile .oauth_wrap .age_area .desc {
    margin-top: 6px
  }

  body.oauth_mobile .oauth_wrap .age_area .break_portrait {
    display: none
  }

  body.oauth_mobile .oauth_wrap .age_area .break_landscape {
    display: block
  }
}
...
@media (orientation: landscape) {
  body.oauth_mobile .oauth_wrap .btn_area .btn {
    padding: 15px 0 14px
  }
...
body.cafe24 .content.start_page {
  box-sizing: border-box;
  height: 100%
}

body.cafe24 .content.start_page .cafe24_content {
  height: 100%
}

body.cafe24 .custom_checkbox {
  display: inline-block;
  position: relative;
  cursor: pointer;
  vertical-align: top
...
body.cafe24 .custom_checkbox.size20 {
  padding-left: 28px
}
...
body.cafe24 .custom_checkbox.size16 {
  padding-left: 24px
}
...
body.cafe24 .custom_checkbox::before {
  position: absolute;
  left: 0;
  content: ''
}
...
.header_cafe24 .login_area {
  margin-top: 10px;
  font-size: 0;
  text-align: right
}

.header_cafe24 .user_id {
  font-size: 13px;
  line-height: 15px;
  color: #929294
}

.header_cafe24 .btn_logout {
  margin-left: 8px;
  padding: 6px 11px;
  border-radius: 14px;
  background: #959eae;
...
.cafe24_content .start_box .image_area {
  margin: -1px -31px 0;
  padding: 30px;
...
.cafe24_content .start_box .notice_area {
  padding: 30px 0
}

.cafe24_content .start_box .cafe_image {
  width: 100%;
  max-width: 230px;
  vertical-align: top
}

.cafe24_content .start_box .title {
  display: block;
  font-size: 16px;
  font-weight: 500;
  line-height: 22px;
...
.cafe24_content .start_box .point {
  font-weight: 700;
  color: #09aa5c
}

.cafe24_content .start_box .list {
  margin-top: 8px
}

.cafe24_content .start_box .item {
  font-size: 15px;
  line-height: 25px;
  color: #929294
}

.cafe24_content .start_box .item::before {
  content: '�� '
}

.cafe24_content .start_box .link_point {
  display: inline-block;
  color: #0459cb;
  text-decoration: underline;
  vertical-align: top
...
.cafe24_content .button_area.no_border {
  border: 0
}

.cafe24_content .button_area .title {
  display: block;
...
.cafe24_content .button_area .button {
  width: 190px;
  padding: 10px 0;
  border-radius: 6px;
  border: 1px solid rgba(0, 0, 0, .05);
...
.cafe24_content .button_area .button.on {
  background: #09aa5c
}

.cafe24_content .button_area .button:disabled {
  background: #c9d1dc;
  cursor: default
}

.cafe24_content .button_area .button+.button {
  margin-left: 10px
}

.cafe24_content .button_area .text_app,
.cafe24_content .button_area .text_talktalk {
  position: relative;
  padding-left: 27px
}
...
.cafe24_content .button_area .agree {
  margin: -16px 0 40px;
  font-size: 15px;
  line-height: 22px;
...
.cafe24_content .talktalk_skip {
  margin-top: 25px;
  text-align: center
}

.cafe24_content .talktalk_skip .btn_skip {
  font-size: 15px;
  font-weight: 500;
  line-height: 22px;
  color: #929294;
...
.cafe24_content .login_other {
  margin-top: 25px;
  text-align: center
}

.cafe24_content .login_other .link {
  display: inline-block;
  font-size: 15px;
  font-weight: 500;
  line-height: 22px;
...
.cafe24_content .order_list {
  display: flex;
  padding: 8px 50px 40px;
  font-size: 0
}

.cafe24_content .order_list.first_on .item.first .order {
  background: #09aa5c
}

.cafe24_content .order_list.first_on .item.first .text {
  font-weight: 700;
  color: #1e1e23
}

.cafe24_content .order_list.second_on .item.first .order {
  background: #09aa5c
}

.cafe24_content .order_list.second_on .item.first .text {
  color: #1e1e23
}

.cafe24_content .order_list.second_on .item.second .order {
  background: #00a48e
}

.cafe24_content .order_list.second_on .item.second .text {
  font-weight: 700;
  color: #1e1e23
}

.cafe24_content .order_list.second_on .line.first {
  background: linear-gradient(to right, #09aa5c 0, #00a48e)
}

.cafe24_content .order_list.third_on .item.first .order {
  background: #09aa5c
}

.cafe24_content .order_list.third_on .item.first .text {
  color: #1e1e23
}

.cafe24_content .order_list.third_on .item.second .order {
  background: #00a48e
}

.cafe24_content .order_list.third_on .item.second .text {
  color: #1e1e23
}

.cafe24_content .order_list.third_on .item.third .order {
  background: #008bc8
}

.cafe24_content .order_list.third_on .item.third .text {
  font-weight: 700;
  color: #1e1e23
}

.cafe24_content .order_list.third_on .line.first {
  background: linear-gradient(to right, #09aa5c 0, #00a48e)
}

.cafe24_content .order_list.third_on .line.second {
  background: linear-gradient(to right, #00a48e 0, #008bc8)
}

.cafe24_content .order_list .item {
  position: relative;
  padding-bottom: 22px;
  color: #929294
}

.cafe24_content .order_list .line {
  flex: 1 1 auto;
  height: 2px;
  margin: 10px 10px 0;
  background: #e3e3e3
...
.cafe24_content .order_list .order {
  display: block;
  width: 20px;
  height: 20px;
  margin: 0 auto;
...
.cafe24_content .order_list .text {
  position: absolute;
  bottom: 0;
  left: 50%;
  font-size: 13px;
...
.cafe24_content .setting_guide+.section {
  padding-top: 30px
}

.cafe24_content .section {
  position: relative;
  padding-top: 20px
}

.cafe24_content .section_title {
  display: block;
  padding-right: 170px;
  font-size: 18px;
  font-weight: 600;
...
.cafe24_content .section_title .point {
  display: inline-block;
  margin-top: -1px;
  font-size: 15px;
  font-weight: 400;
...
.cafe24_content .btn_register .text {
  position: relative;
  padding-left: 27px;
  font-size: 14px;
  font-weight: 500;
...
.cafe24_content .table_area {
  position: relative;
  margin-top: 29px
}

.cafe24_content .table_header {
  display: flex;
...
.cafe24_content .table_header .cell {
  padding: 15px 0
}

.cafe24_content .table_header .text {
  font-size: 14px;
  font-weight: 500;
  line-height: 17px;
  color: #929294
...
.cafe24_content .table_list {
  overflow: auto;
  max-height: 261px
}

.cafe24_content .table_list .item {
  position: relative
}

.cafe24_content .table_list .item+.item {
  border-top: 1px solid #f1f2f6
}

.cafe24_content .table_list .item:last-of-type {
  border-bottom: 1px solid #d3d4d8
}

.cafe24_content .table_list .cell.type_logo {
  padding: 24px 0
}

.cafe24_content .table_list .item_inner {
  display: flex
}

.cafe24_content .table_list input:checked+label .box {
  border-color: #09aa5c
}

.cafe24_content .table_list input:checked+label .box::before {
  position: absolute;
  top: 3px;
  right: 3px;
  bottom: 3px;
...
.cafe24_content .table_list label {
  display: flex
}

.cafe24_content .table_list label:hover {
  background: rgba(149, 158, 174, .05)
}

.cafe24_content .table_list label:hover .box {
  border-color: #09aa5c
}
...
.cafe24_content .cell.type_select {
  flex: none;
  width: 80px;
  text-align: center
}

.cafe24_content .cell.type_logo {
  flex: none;
  display: flex;
  align-items: center;
  justify-content: center;
...
.cafe24_content .cell.type_app_name {
  flex: none;
  width: 174px;
  margin-left: 20px
}

.cafe24_content .cell.type_client_id {
  flex: none;
  width: 174px;
  margin-left: 20px
}

.cafe24_content .cell.type_create_date {
  flex: 1 1 auto;
  margin-left: 20px
}

.cafe24_content .cell.type_private_info {
  flex: 1 1 auto;
  margin-left: 20px
}

.cafe24_content .app_logo {
  width: 38px;
  height: 38px
}

.cafe24_content .cell_text {
  display: inline-block;
  font-size: 15px;
  line-height: 22px;
  color: #1e1e23;
...
.cafe24_content .max_three_lines {
  display: -webkit-box;
  -webkit-line-clamp: 3;
...
.cafe24_content .register_box {
  padding: 20px 0 30px
}

.cafe24_content .register_box .register_line {
  padding: 10px 0
}

.cafe24_content .register_box .register_line::after {
  display: block;
  clear: both;
  content: ''
}

.cafe24_content .register_box .title_area {
  position: relative;
  float: left;
  width: 166px;
  padding-top: 9px;
...
.cafe24_content .register_box .title {
  font-size: 15px;
  line-height: 22px;
  color: #767678
}

.cafe24_content .register_box .desc_area {
  overflow: hidden
}

.cafe24_content .register_box .btn_info {
  padding: 1px;
  vertical-align: top
}

.cafe24_content .register_box .btn_info[aria-expanded=true]+.info_layer {
  display: block
}
...
.cafe24_content .register_box .info_layer {
  display: none;
  position: absolute;
  top: 36px;
  left: 0;
...
.cafe24_content .register_box .btn_layer_close {
  position: absolute;
  top: 10px;
  right: 15px
}
...
.cafe24_content .register_box .text_input {
  box-sizing: border-box;
  width: 100%;
...
.cafe24_content .register_box .text_input:not(:-moz-read-only):focus,
.cafe24_content .register_box .text_input:not(:-moz-read-only):hover {
  border-color: #09aa5c
}

.cafe24_content .register_box .text_input:not(:read-only):focus,
.cafe24_content .register_box .text_input:not(:read-only):hover {
  border-color: #09aa5c
}

.cafe24_content .register_box .text_input.error {
  border-color: #fe4638
}

.cafe24_content .register_box .label_file {
  display: inline-block;
...
.cafe24_content .register_box .label_file .text {
  position: relative;
  padding-left: 23px;
  font-size: 15px;
  font-weight: 500;
...
.cafe24_content .register_box .logo_area img {
  vertical-align: top
}

.cafe24_content .register_box .logo_box {
  display: inline-block;
  overflow: hidden;
  position: relative;
  vertical-align: top
...
.cafe24_content .register_box .logo_box::after {
  position: absolute;
  top: 0;
  right: 0;
  bottom: 0;
...
.cafe24_content .register_box .btn_delete_file {
  margin-top: 10px;
  padding: 10px 19px 9px 17px;
  border-radius: 4px;
  border: 1px solid #424242;
...
.cafe24_content .register_box .btn_delete_file .text {
  position: relative;
  padding-left: 23px;
  font-size: 15px;
  font-weight: 500;
...
.cafe24_content .notice_list {
  padding-top: 12px
}

.cafe24_content .notice_list+.section_title {
  margin-top: 50px
}

.cafe24_content .notice_list .item {
  position: relative;
  font-size: 14px;
  line-height: 22px;
  color: #929294
...
.cafe24_content .notice_list .item.error {
  color: #fe4638
}

.cafe24_content .notice_list .item a {
  color: inherit;
  text-decoration: underline
}

.cafe24_content .talktalk_area {
  position: absolute;
  top: 140px;
  right: 0;
  bottom: 0;
...
.cafe24_content .talktalk_info_list {
  padding: 10px 0 40px
}

.cafe24_content .talktalk_info_list .item {
  padding-top: 20px
}

.cafe24_content .talktalk_info_list .item::after {
  display: block;
  clear: both;
  content: ''
}

.cafe24_content .talktalk_info_list .title {
  float: left;
  width: 132px;
  font-size: 15px;
  font-weight: 400;
...
.cafe24_content .talktalk_info_list .desc {
  overflow: hidden;
  font-size: 15px;
  line-height: 22px;
  color: #1e1e23;
...
.cafe24_content .talktalk_info_list .link {
  color: #3972e9
}
...
.cafe24_footer .item+.item {
  padding-left: 1px
}

.cafe24_footer .item+.item::before {
  position: absolute;
  top: 15px;
  left: 0;
  width: 1px;
  height: 12px;
  background: rgba(125, 135, 156, .15);
  content: ''
}
...
.header_blank.scrolled {
  border-bottom: 1px solid #efeff0
}

.header_blank.scrolled::after {
  position: absolute;
  right: 0;
  bottom: -9px;
  left: 0;
...
/* 기본 탭 */
.menu_id[data-v-e4b3ab34],
.menu_ones[data-v-e4b3ab34],
.menu_qr[data-v-e4b3ab34] {
  display: block;
  position: relative;
  height: 63px;
  padding-top: 12px;
...
/* 활성화된 탭 */
.menu_id.on[data-v-e4b3ab34],
.menu_ones.on[data-v-e4b3ab34],
